...
- `create --approve` creates through the delegated-approval endpoint (`/spend_requests/create_delegated` — see `SpendRequestResource.create()` in the SDK), returning an already-approved request unless step-up is required. Requires an OAuth client authorized for `spend_requests:approve` (separate from `payment_methods.agentic`); the backend allows delegated creation only for specifically granted partner OAuth apps. With `--execution-method link_pay_token`, `--approve` requires `--no-request-approval` — the CLI errors otherwise rather than silently overriding the approval-mode choice, since the two modes are mutually exclusive (delegated vs. consumer-polled).
