...
- `mpp pay <url> --context <ctx> [-X <method>] [-d <body>] [-H <header>]... [--amount <cents>] [--payment-method-id <id>] [--test]` — handles the full MPP flow end-to-end. Before the 402, it can answer a Link identity 401 carrying `PrivateToken` and/or `Identity-Presentation`: it validates that a claims challenge targets the effective request origin, supports `dc+sd-jwt`, and trusts `https://api.link.com`; creates the identity presentation before destructively taking a pooled attestation; and retries without following redirects. Since that retry can consume the identity nonce while unlocking the 402, payment submission strips the used access headers, fetches one fresh 401, builds fresh proofs without sending them alone, and attaches those proofs together with the payment credential to the final request. It parses the resulting 402 to extract network_id and amount, creates a spend request (credential_type: shared_payment_token), gets user approval, retrieves the SPT, and pays. Amount/currency are derived from the 402 challenge; `--amount` overrides. `--context` is required (min 100 chars) — describe the purchase and rationale. Default payment method is used unless `--payment-method-id` is specified.
