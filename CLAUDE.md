...
- **`@stripe/link-integrations-eve`** (`packages/integrations/eve`): Native Eve extension built with `eve extension build`. Static tool files wrap `@stripe/link-sdk/tools` and use the configured `accessToken` directly, without OAuth or automatic refresh. Maintain its custom `extension/skills/link-wallet/SKILL.md` alongside the tool behavior. Workspace development and CI require Node 24+.
