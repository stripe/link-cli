...
- `mpp pay <url> --context <ctx> [-X <method>] [-d <body>] [-H <header>]... [--identity-claim <claim>]... [--amount <cents>] [--payment-method-id <id>] [--test]` — handles the full MPP flow end-to-end. When `LINK_IDENTITY_COMMANDS=1`, it can answer a Link identity 401 carrying `PrivateToken` and/or `Identity-Presentation`; with the flag unset, it neither consumes identity artifacts nor accepts `--identity-claim`. The enabled flow validates that a claims challenge targets the effective request origin, supports `dc+sd-jwt`, and trusts `https://api.link.com`; creates the identity presentation before destructively taking a pooled attestation; and retries without following redirects. Identity disclosure is opt-in: each requested claim must appear in a repeatable `--identity-claim` allowlist bound to the original URL origin, and that allowlist is preserved in approval continuations and enforced on every refreshed challenge. Since an identity retry can consume its one-time credentials while unlocking the 402, each continuation strips the used access headers, fetches one fresh 401, builds fresh proofs without sending them alone, and attaches those proofs together with the payment credential to the final request. It parses the resulting 402 to extract network_id and amount, creates a spend request (credential_type: shared_payment_token), gets user approval, retrieves the SPT, and pays. Amount/currency are derived from the 402 challenge; `--amount` overrides. `--context` is required (min 100 chars) — describe the purchase and rationale. Default payment method is used unless `--payment-method-id` is specified.
...
- Generated `Authorization: PrivateToken` and `Identity-Presentation` values are ephemeral secrets and must never be included in `_next.pay_argv` or `_next.pay_command`. The continuation obtains a fresh 401 challenge and new credentials after approval. If `Authorization` carries `PrivateToken`, the 402 challenge must select a separate payment credential header; fail rather than overwrite the attestation.
