...
async function nodeRequestToWebRequest(req: IncomingMessage, port: number): Promise<Request> {
...
export function createServeCli(rootCli: { fetch: (req: Request) => Promise<Response> }) {
  return Cli.create('serve', {
    ...
    async run(c) {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        server.listen(port, () => {
          process.stderr.write(`link-cli MCP server listening on http://localhost:${port}/mcp\n`);
        });
      });
    },
  });
