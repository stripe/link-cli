...
export class BalancesResource implements IBalancesResource {
  ...
  constructor(options: LinkOptions = {}) {
    ...
    this.endpoint = `${config.apiBaseUrl}/balances`;
  ...
  async listBalances(params: ListBalancesParams = {}): Promise<BalancesPage> {
    ...
    if (status < 200 || status >= 300) {
      ...
      throw new LinkApiError(`Failed to list balances (${status}): ${msg}`, {
        status,
        rawBody,
        details: data,
      });
    ...
    try {
      ...
      throw new LinkApiError(
        `Failed to list balances (200): invalid response shape${reason}`,
